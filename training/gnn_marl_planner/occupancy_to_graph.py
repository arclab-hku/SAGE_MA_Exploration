...
class occupancy_to_graph(Node):
    def __init__(self):
        ...
        self.subscription_update_range = self.create_subscription(
            Odometry,
            "/state_ukf/odom_1",
            self.update_viewrange_callback,
            10)
        # 地图参数
        ...
        self.map_x = None # UAV 的位置
        self.map_y = None # UAV 的位置
        # 拓扑图相关的参数
    ...
    def occupancy_to_rgb(self, data, bottom, top, left, right, circle_mask):
    ...
    def update_viewrange_callback(self, msg):  
        if self.map_data is None:
            return
        ros_x = msg.pose.pose.position.x
        ros_y = msg.pose.pose.position.y
        self.get_logger().info(f'ros_x: {ros_x}, ros_y: {ros_y}')
        # 反转先前转换的计算，计算2D地图坐标
        self.map_x = int((-ros_x + self.map_origin[1]) / (self.resolution * self.downsample_factor))
        self.map_y = int((-ros_y + self.map_origin[0]) / (self.resolution * self.downsample_factor))
        
        # 计算方框的边界
        radius = self.viewrange
        left = int(max(0, self.map_y - radius))
        right = int(min(self.width, self.map_y + radius + 1))
        bottom = int(max(0, self.map_x - radius))
        top = int(min(self.height, self.map_x + radius + 1))

        # 创建局部坐标系
        y, x = np.ogrid[left:right, bottom:top,]
        
        # 计算到中心的距离
        dist_from_center = np.sqrt((x - self.map_x)**2 + (y - self.map_y)**2)
        ...
        # self.get_logger().info(f'frontier_dict: {self.frontier_dict}')
        self.update_viewpoints(self.map_x, self.map_y)            
        self.delete_view_point() # 只要一次就够
        
        self.occupancy_to_rgb(self.map_data, bottom, top, left, right, circle_mask)
    ...
    def delete_view_point(self):
        ...
        for i in range(self.drone_num):
            if (self.map_x, self.map_y) in self.viewpoint_dict:
                delete_item.append((self.map_x, self.map_y))
        ...
        for i in range(self.drone_num):
            min_x = max(0, self.map_x-1)
            min_y = max(0, self.map_y-1)
            max_x = min(self.map_size_height, self.map_x+1)
            max_y = min(self.map_size_width, self.map_y+1)
            # ii, jj = np.mgrid[min_x:max_x+1, min_y:max_y+1]
            # candidate_view_points = np.stack([ii.ravel(), jj.ravel()], axis=-1)
            candidate_view_points = [(min_x, self.map_y), (max_x, self.map_y), (self.map_x, min_y), (self.map_x, max_y)]
            for (nx, ny) in candidate_view_points:
